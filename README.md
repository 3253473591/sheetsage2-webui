...
把一首歌的录音丢进去，自动扒出**主旋律 / 器乐 / 和弦 / 节拍 / 调性 / 段落**，在**钢琴卷帘**上改音符与歌词，并导出成 **Synthesizer V 工程（`.svp`）/ 通用 MIDI（`.mid`）**。
...
| 输出 | SVP 或 MIDI（通常单旋律） | **SVP + MIDI**，人声主旋律 / 器乐旋律 / 和弦**多轨可选** |
| 编辑闭环 | 无 | **钢琴卷帘**在线编辑 → 重新生成 → 离线钢琴试听（**音符高亮**）→ 导出 |
...
> 📺 **使用教程：[原曲丢进去自动出谱，妈妈再也不用担心我的扒谱【SheetSage2 WebUI使用教程】](https://www.bilibili.com/video/BV1Xeh86JEGs/)**（2 分半，从拖入音频讲到导出 SVP）

**网盘下载：<https://pan.baidu.com/s/1arnZDflfge6Pl8hqoPXkoA?pwd=9ee5>**
（提取码 `9ee5`，与视频置顶评论一致；若链接失效请以**视频置顶评论**为准。）
...
```
拖入音频 → 推理（GPU 或 CPU）→ 分析概览 → 钢琴卷帘（改音符 / 填歌词）→ 导出
                                  ↑                                ↓
                          调性/速度/段落/和弦进行            .svp / .mid
```
